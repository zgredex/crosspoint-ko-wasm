...
int main(int argc, char** argv) {
  ...
  // The general invariant: the advance must be fractional. If it is a whole number of pixels the
  // header was regenerated by something that rounded, and every wrap in the book may move.
  if (hangul.advanceX == 0 || hangul.advanceX % 16 == 0) {
    std::fprintf(stderr, "FAIL: U+AC00 advanceX is %u = %u px exactly; this face has lost its "
                         "fractional advances and cannot be the ko fork's build\n",
                 hangul.advanceX, hangul.advanceX / 16);
    return 1;
  }
  if (expect != nullptr && hangul.advanceX != expect->uac00Advance) {
