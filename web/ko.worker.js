...
// compiled in, and staged next to the module — so a face can be fetched in PARALLEL with the module
// instead of discovered missing after it is ready. Guarded for the same reason assets.js is: not
// every build emits it, and a missing file must not kill the worker before it can fall back.
try {
  importScripts('ko_build_info.js' + self.location.search);
} catch (_) { /* no build info: assume everything is embedded (see embeddedFaces) */ }
